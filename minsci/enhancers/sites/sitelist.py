...
class SiteList(MutableSequence):
    ...
    def match(self, name=None, site=None, attr=None, syndex=3):
        ...
        print('Matching {}={}...'.format(attr if attr else 'val', name))
        while True:
    ...
    def _match_name(self, name, attr=None):
        ...
        for site in self:
            names = [self.std(n) for n in site.site_names + site.synonyms]
            matched = name in names
            if matched:
                matches.append(site)
            stnames = str(names)[:40] + '...'
            print(" '{}'".format(name), 'in' if matched else 'not in', stnames)
        self._obj = matches
        self.filters.extend([{'_name': name}, {attr: 1}])
        return self
    ...
    def match_attr(self, site, attr):
        ...
        self.filters.append({attr: maxscore})
        if maxscore >= 0:
